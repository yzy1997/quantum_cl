...
def U3_layer(nqubits):
    ...
    """
    for idx in range(nqubits):
        qml.U3(np.pi/2, np.pi , 3*np.pi/2 , wires=idx)
...
class DressedQuantumNet(nn.Module):
    ...
    def forward(self, input_features):
        ...
        print("================the end of normal net===================")
        for elem in q_in:
            q_out_elem = torch.hstack(quantum_net_2(elem, self.q_params)).float().unsqueeze(0)
...
plt.tight_layout()
plt.savefig('./results/lwf_tiny_imagenet_quantum_qnet2.png')
plt.show()

...
class DressedQuantumNet(nn.Module):
    ...
    def forward(self, input_features):
        ...
        q_out = torch.zeros(0, n_qubits,requires_grad=True)
        q_out = q_out.to(device)
        for elem in q_in:
            q_out_elem = torch.hstack(quantum_net_4(elem, self.q_params)).float().unsqueeze(0)
...
plt.tight_layout()
plt.savefig('./results/lwf_tiny_imagenet_quantum_qnet4.png')
plt.show()
